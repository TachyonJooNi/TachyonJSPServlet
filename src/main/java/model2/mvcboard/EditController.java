...
public class EditController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		//디렉토리의 물리적 경로 얻어오기
		//-- 02ImplicitObject.ApplicationMain.jsp 교안에 물리적경로
		//-- 받아오는 여러 방법 설명있음.
		...
		/*
		패스워드의 경우 인증이 완료되었을떄 session영역에 저장해 두었다.
		...
		*/
		...
		if (fileName != null) {
			...
			//-- 만약 여기서 기존 파일명을 가지고있지 않았다면 updatePost()의 쿼리문에
			//-- if문으로 등록여부를 확인하면서 처리해야하는 번거로움이 있었을것이다.
		...
		if (result == 1) {
			//수정이 완려되었다면 session에 저장된 패스워드를 지운후 상세보기 화면으로 이동한다.
