...
public class PassController extends HttpServlet{
	...
	//패스워드 검증 페이지로 진입하는 역할만함
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
			throws ServletException, IOException {
		/*
			/mvcboard/pass.do?mode=delete&idx=135
			요청명은 이와 같으므로 파라미터를 받아오는 작업을 해야한다.
			서블릿에서는 2가지 방법으로 받을수 있는데
			방법 1 : request내장객체(req)를 이용한다. => req.getParameter(파라미터명)
			방법 2 : View(JSP파일)에서 EL의 param 내장객체를 사용한다. => ${param.파라미터명}
			--방법1은 컨트롤러에서 사용해야하는 경우, 방법2는 뷰에서 사용해야하는 경우
		*/
